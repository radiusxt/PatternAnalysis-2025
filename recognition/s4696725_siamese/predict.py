...
def compute_class_prototypes(model, device, image_dir, csv_path, num_per_class: int = 200):
    ...
    with torch.no_grad():
        for img, labels in tqdm(loader, leave=False):
            img = img.to(device)
            emb = model.embedding_net(img)
            
            for i, label in enumerate(labels):
                embeddings[label.item()].append(emb[i])
...
def predict_on_test(model, device, test_image_dir, test_csv=None, prototypes=None):
    ...
    with torch.no_grad():
        ...
        for imgs, fns, _ in tqdm(loader, leave=False):
            imgs = imgs.to(device)
            ...
            # convert to probability of class 1 (malignant) by softmax over negative distance
